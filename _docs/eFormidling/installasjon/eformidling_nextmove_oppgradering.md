...
2. Her må du gjøre et av stegene under:
- I mappen med integrasjonspunktet: Slett alle gamle databasefiler. Dette kan være én eller flere av følgende:  ```integrasjonspunkt.mv.db``` , ```receipt.mv.db``` og ```receipt.trace.db```, det spørst hvilken versjon du har. 
- **NB!** Om du ikke har disse filene og heller bruker en egen database(feks MySQL) må du slette denne og peike integrasjonspunktet til en ny database. Legg også inn ```spring.jpa.properties.hibernate.jdbc.time_zone=Europe/Oslo``` i *integrasjonspunkt-local.properties* filen.
