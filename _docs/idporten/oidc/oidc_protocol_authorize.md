...
The client passes an authentication request by redirecting the end user's browser to the /authorize endpoint.
...
| redirect\_uri | Required | The end user will be redirected here after a successful authentication.  Only pre-registered URIs can be used.  Localhost is allowed only in test environment. Max 1024 characters.                                                                                                                                                                                                                       | 
| scope |  Required | Whitespace-separated list of requested scopes.  Normally just `openid`.                                                                                                                                                                                                                                                                                                                                   |
| state | Recommended | Value set by the client and returned in the callback. If PKCE is not used, then state must be used to achieve CSRF-protection. Mandatory to use for public clients. Max size is 1024, but for form_post response it is 8192. The server also rejects state values that are malformed or that contain patterns likely to be blocked or altered by intermediaries such as Web Application Firewalls (WAFs). |
| nonce | Recommended | Value set by the client and returned in the id-token. Recommended to use to protect from replay attacks. Max 128 characters. Characters restricted to BASE64URL and BASE64-set characters and the dot (.) character.                                                                                                                                                                                      |
| acr\_values | Optional | Requested security level of assurance(s). See [id_token docs](oidc_protocol_id_token#acr-values). (e.g. `idporten-loa-substantial`)                                                                                                                                                                                                                                                                       |
