...
Merk tilslutt at en enkelt bruker bare kan ha en autorisasjon mot samme klient i gangen.  Dersom klienten har en gyldig autorisasjon med gitte scopes, og så utfører en ny autorisasjon med andre scopes, så vil nye access_token bare innehold scopene fra den nyeste autorisasjonen.  ID-porten "husker" altså ikke samtykkede scopes over flere autorisasjoner,  men dette vil trolig endres i 2026, se [roadmap #264](https://github.com/orgs/digdir/projects/8/views/3?pane=issue&itemId=39922581&issue=digdir%7Croadmap%7C264&reload=1), som vil innføre følgende funksjonalitet:
> ID-porten husker scopes som innbygger tidlegere har samtykket til. Dersom klienten gjennomfører ny innlogging og ber om et scope der det allerede finnes et aktivt samtykke, vil tilgangsdialogen ikke bli vist.  Levetiden til scopet følger av klienten sin konfigurasjon for `authorization_lifetime`, dog beskranket av scope-eiers eventuelle bruk av `authorization_max_lifetime` i [scope-definisjonen](https://docs.digdir.no/docs/idporten/oidc/oidc_protocol_scope.html).  Dersom flere scopes blir forespurt i samme autorisasjon, er det scopet med kortest max-levetid som bestemmer. 
